...
class HGPIFuNet(BasePIFuNet):
    ...
    def __init__(self, opt, projection_mode='orthogonal', error_term=nn.MSELoss(), error_term_semantic=nn.MSELoss()):
        ...
        super(HGPIFuNet, self).__init__(projection_mode=projection_mode, error_term=error_term, error_term_semantic=error_term_semantic)
    ...
    def query(self, points, calibs, transforms=None, labels=None, deepVoxels=None, semantic=None):
        ...
        if self.opt.deepVoxels_fusion == "multiLoss":
            ...
            for im_feat in self.im_feat_list:
                ...
                # 2023/05/15, we add the semantic in this code
                point_local_feat_list = [self.index(im_feat, xy), z_feat, semantic] # torch.nn.functional.grid_sample, in geometry.py
                ...
                pred = in_img[:,None].float() * self.surface_classifier(point_local_feat) # (B, 4, n_in+n_out) == (B, 1, n_in+n_out) * {(B, 1, n_in+n_out), num_views are canceled out by mean pooling}
                self.intermediate_preds_list.append(pred)  # the predict of first line is occpancy and 2-4 is semantic;

        # at inference time, we only rely on the last layer features from stacked-hour-glass-networks
        self.preds = self.intermediate_preds_list[-1] # (B * num_view, 4, n_in+n_out)
    ...
    def get_error(self):
        ...
        self.label :   is label of each point , [b, 1, n]
        '''
        ...
        for preds in self.intermediate_preds_list:
            ...
            preds_2 = preds[:, 0:1, :]  # [b, 1, n] , the first line of predicts is occpancy_data ;

            if self.opt.occupancy_loss_type == "ce":

                # occupancy CE loss, random baseline is: (1000. * -np.log(0.5) / 2. == 346.574), optimal is: 0.
                w = 0.7
                error += 1000. * (   -w  * torch.mean(    self.labels * torch.log(  preds_2+1e-8)) # preds: (B,1,5000), self.labels: (B,1,5000)
                                  -(1-w) * torch.mean((1-self.labels) * torch.log(1-preds_2+1e-8))
                                 ) # R
            else:

                error += self.error_term(preds_2, self.labels)  # default is nn.MSELoss()
    ...
    def get_semantic_error(self, semantic):
        '''
        Hourglass has its own intermediate supervision scheme
        semantic is label of this point!     ;   [b, 3, n_in+n_out]
        '''
        ...
        for preds in self.intermediate_preds_list:
            ...
            preds_2 = preds[:, 1:4, :]  # [b, 3, n] the 2-4 line of predicts is semantic_data ;

            error += self.error_term_semantic(preds_2, semantic)  # default is nn.MSELoss()
            # according to the train_normal, we can see , the loss function only need keep dim is same is okay!

        # average loss over different latent feature layers
        error /= len(self.intermediate_preds_list)

        return error
    def forward(self, images, points, calibs, transforms=None, labels=None, deepVoxels=None, semantic_tensor=None):
        """
        input
            ...
            semantic_tensor  :  (b, 3, n_in+n_out)
        ...
        # Phase 2: point query
        self.query(points=points, calibs=calibs, transforms=transforms, labels=labels, deepVoxels=deepVoxels, semantic= semantic_tensor)

        # Phase 3: get the prediction
        res = self.get_preds() # return self.preds,  the predicts is (B, 4, n_in+n_out) ; and the first line is occpancy and 2-4line is semantic data
        
        # Phase 4: get the error, default is nn.MSELoss()
        error = self.get_error() # Return occupancy loss ;
        semantic_error = self.get_semantic_error(semantic_tensor) # return semantic loss ;

        # reference Train_Normal!
        all_error = error + 0.7*semantic_error

        return res, all_error  # the predicts include occupancy_predict and semantic_predicts!  Additionally in the first line and 2-3-4 line
