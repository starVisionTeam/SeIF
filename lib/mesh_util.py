...
def reconstruction_iccv(net, cuda, calib_tensor, resolution_x, resolution_y, resolution_z, b_min, b_max, use_octree=False, num_samples=70000, transform=None, deepVoxels=None):
    '''
    ...
    try:
        # debug!!! module 'skimage.measure' has no attribute 'marching_cubes_lewiner'
...
def reconstruction(netG_SeIF, netG_Semantic, cuda, calib_tensor, resolution, b_min, b_max, use_octree=False, num_samples=10000, transform=None):
    '''
    Reconstruct meshes from sdf predicted by the network.
    :param net: a BasePixImpNet object. call image filter beforehead.
    :param cuda: cuda device
    ...
    def eval_func(points):
        points = np.expand_dims(points, axis=0)  # (1,         3, num_samples)
        points = np.repeat(points, netG_SeIF.num_views, axis=0)  # (num_views, 3, num_samples)
        samples = torch.from_numpy(points).to(device=cuda).float()  # (num_views, 3, num_samples)
        netG_Semantic.query(samples, calib_tensor)  # calib_tensor is (num_views, 4, 4)
        # before is [b, 1, n] , now is [b,4,n], and the first line is occ of predict!
        semantic = netG_Semantic.get_preds()[:, 1:4, :]  # Se:  we get the semantic of samples       # (b, 3, num_samples)

        netG_SeIF.query(samples, calib_tensor, semantic=semantic)
        pred = netG_SeIF.get_preds()[:, 0:1, :][0][0]  # OCC: we get the occ of samples;
        return pred.detach().cpu().numpy()

        # Then we evaluate the grid, use_octree default: True

    if use_octree:
        sdf = eval_grid_octree(coords, eval_func,
                               num_samples=num_samples)  # XYZ, WHD, (256, 256, 256), float 0. ~ 1. for occupancy
    else:
