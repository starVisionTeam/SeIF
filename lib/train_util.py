...
def reshape_multiview_tensors(image_tensor, calib_tensor, normal_tensor ):
    ...
    normal_tensor = normal_tensor.view(
        normal_tensor.shape[0] * normal_tensor.shape[1],
        normal_tensor.shape[2],
        normal_tensor.shape[3],
        normal_tensor.shape[4]
    )
    ...
    return image_tensor, calib_tensor, normal_tensor
...
def gen_mesh_iccv(opt, net, cuda, data, save_path, use_octree=True):
    ...
    image_tensor = torch.cat([image_tensor, normal_tensor], dim=1)  # [1 , 6 , 512 ,512]
    ...
    if opt.deepVoxels_fusion != None: deepVoxels_tensor = data["deepVoxels"].to(device=cuda) # (B=1,C=8,D=32,H=48,W=32), np.float32, all >= 0.
    ...
    try:
        ...
        # xyz_tensor *= torch.Tensor([[1.0],[-1.0],[-1.0]]).to(device=cuda).float()
        uv = xyz_tensor[:, :2, :] # (1, 2, N) for xy, float -1 ~ 1
        color = index(image_tensor[:1], uv).detach().cpu().numpy()[0].T # (N, 3), RGB, float -1 ~ 1
        color = color * 0.5 + 0.5 # (N, 3), RGB, float 0 ~ 1
        save_obj_mesh_with_color(save_path, verts, faces, color)
...
# generate proj-color-mesh for a data point (can be multi-view input)
def gen_mesh(opt, netG_SeIF, netG_Semantic, cuda, data, save_path, use_octree=True):

    image_tensor = data['img'].to(device=cuda)  # (num_views, 3, 512, 512)
    image = image_tensor
    normal_tensor = data['normal'].to(device=cuda)

    # before send image to filter ,we should visualize it to see the background is the same
    visualize = False
    if visualize:
        img = np.uint8((np.transpose(image_tensor[0].cpu().numpy(), (1, 2, 0)) * 0.5 + 0.5)[:, :,
                       ::-1] * 255.0)  # HWC, BGR, from -1,1 to float 0 ~ 255, de-normalized by mean 0.5 and std 0.5
        cv2.imwrite('/home/lx/29_0021_withNOrmal/img.jpg', img)
        img2 = np.uint8((np.transpose(normal_tensor[0].cpu().numpy(), (1, 2, 0)) * 0.5 + 0.5)[:, :,
                        ::-1] * 255.0)  # HWC, BGR, from -1,1 to float 0 ~ 255, de-normalized by mean 0.5 and std 0.5
        cv2.imwrite('/home/lx/29_0021_withNOrmal/img_normal.jpg', img2)

    image_tensor = torch.cat([image_tensor, normal_tensor], dim=1)  # in channel connect with each photo
    calib_tensor = data['calib'].to(device=cuda)  # (num_views, 4, 4)

    # use hour-glass networks to extract image features
    netG_SeIF.filter(image_tensor)
    netG_Semantic.filter(image_tensor)
    b_min = np.array([-0.5, -0.5, -0.5])
    b_max = np.array([0.5, 0.5, 0.5])
    # the volume of query space
    ...
    try:
        ...
        for v in range(image.shape[0]):
            save_img = (np.transpose(image[v].detach().cpu().numpy(), (1, 2, 0)) * 0.5 + 0.5)[:, :,
        ...
        save_img_list = []
        save_img_path = save_path[:-4] + '_normal.png'
        for v in range(normal_tensor.shape[0]):
            save_img = (np.transpose(normal_tensor[v].detach().cpu().numpy(), (1, 2, 0)) * 0.5 + 0.5)[:, :,
                       ::-1] * 255.0  # RGB -> BGR, (3,512,512), [0, 255]
            save_img_list.append(save_img)
        save_img = np.concatenate(save_img_list, axis=1)
        ...
        # verts: (N, 3) in the mesh-coords, the same coord as data["samples"], N << 256*256*256
        # faces: (N, 3)
        verts, faces, _, _ = reconstruction(netG_SeIF, netG_Semantic, cuda, calib_tensor, opt.resolution, b_min, b_max,
                                            use_octree=use_octree)
        verts_tensor = torch.from_numpy(verts.T).unsqueeze(0).to(device=cuda).float()  # (1, N, 3)
        xyz_tensor = netG_SeIF.projection(verts_tensor, calib_tensor[
                                                  :1])  # （1, 3, N） Tensor of xyz coordinates in the image plane of (-1,1) zone and of the-first-view
        uv = xyz_tensor[:, :2, :]  # (1, 2, N) for xy, float -1 ~ 1
        color = torch.ones((uv.shape[2], 3)).numpy()
        # # color = index(image[:1], uv).detach().cpu().numpy()[0].T  # (N, 3), RGB, float -1 ~ 1
        # color = color * 0.5  # (N, 3), RGB, float 0 ~ 1
        color = color*(203/255)
        save_obj_mesh_with_color(save_path, verts, faces, color)

        ms = ml.MeshSet()
        ms.load_new_mesh(save_path)
        # use this point all linjie point, add_weight average location as it new location!
        ms.apply_coord_laplacian_smoothing(stepsmoothnum=3, boundary=True, cotangentweight=True, selected=False)
...
def calc_error(opt, net, cuda, dataset, num_tests):
    ...
    with torch.no_grad():
        ...
        for idx in range(num_tests):
            # retrieve data for one frame (or multi-view)
            data = dataset[idx * len(dataset) // num_tests]  # data is a dict

            image_tensor  = data['img'].to(device=cuda)                  # (num_views, C, W, H) for 3x512x512 images, float -1. ~ 1.
            normal_tensor = data['normal'].to(device=cuda)
            image_tensor = torch.cat([image_tensor, normal_tensor], dim=1)  # [n , 6 , h, w ]
            ...
            semantic_tensor = data['semantic'].to(device=cuda).unsqueeze(0)
            ...
            # forward pass
            res, error = net.forward(image_tensor, sample_tensor, calib_tensor, labels=label_tensor, deepVoxels=deepVoxels_tensor, semantic_tensor=semantic_tensor) # (1, 1, n_in + n_out), R
            if len(opt.gpu_ids) > 1: error = error.mean()  # the error is occupancy loss and semantic loss!

            res = res[:, 0:1, :]   # the first line is this points predicts of occupancy! and the 2-4 line is semantic data of predict!
