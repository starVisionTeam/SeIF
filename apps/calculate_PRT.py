...
def comPRT(obj_path, path_save_prt,
           n=40):  # obj_path =  /media/amax/4C76448F76447C28/Dataset/COMA_norm/people1/bareteeth.00001.obj
    PRT, F = computePRT(obj_path, n, 2)

    _, houmian = os.path.split(obj_path)  # houmian = bareteeth.00001.obj
    qianmian, _ = os.path.splitext(houmian)  # qianmian = bareteeth.000001

    new_path = os.path.join(path_save_prt,
                            qianmian)  # new_path = /media/amax/4C76448F76447C28/Dataset/PRT/people1/bareteeth.00001
    new_path = new_path + '.mat'  # new_path= /media/amax/4C76448F76447C28/Dataset/PRT/people1/bareteeth.00001.mat
...
if __name__ == '__main__':
    ...
    root = '/media/amax/4C76448F76447C28/FullData/facescape_norm'
    root_prt = '/media/amax/4C76448F76447C28/FullData/PRT'
    for file in np.arange(800, 850):
        file = str(file)
        path_obj = os.path.join(root, file)  # '/media/amax/4C76448F76447C28/FullData/facescape_norm/1'
        path_save_prt = os.path.join(root_prt, file)  # '/media/amax/4C76448F76447C28/FullData/PRT/1'

        if os.path.exists(path_obj):   # if this facescape_norm path is exits ;
            if not os.path.exists(path_save_prt):  # if prt_save is not exits , create it and write to it. or pass it
                os.makedirs(path_save_prt, exist_ok=True)
                print(path_obj)
                print(path_save_prt)

                for filename in os.listdir(path_obj):
                    qianzhui, houzhui = os.path.splitext(filename)
                    if (houzhui == '.obj'):
                        pathTmep = os.path.join(path_obj, filename)
                        comPRT(pathTmep, path_save_prt)

                print('file = ', file)

    # path_obj = '/media/amax/4C76448F76447C28/FullData/facescape_norm/1'
    # path_save_prt = '/media/amax/4C76448F76447C28/FullData/PRT/1'
    # for filename in os.listdir(path_obj):
    #     qianzhui, houzhui = os.path.splitext(filename)
    #     if(houzhui=='.obj'):
    #         print(i)
    #         pathTmep = os.path.join(path_obj, filename)  # /media/amax/4C76448F76447C28/Dataset/COMA_norm/people1/bareteeth.00001.obj
    #         comPRT(pathTmep, path_save_prt)
    #         i = i +1
