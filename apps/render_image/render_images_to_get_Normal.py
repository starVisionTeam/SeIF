...
def read_data(item, prt_file):
    ...
    prt_filename = os.path.join(prt_file, houmian)
...
def main():
    i = 53
    j = i + 5
    data_root = r'D:\Faceverse\normalization_test'
    prt_root = r'D:\Faceverse\PRT'
    save_data_file_root = r'E:\Faceverse\Render'
    for file in np.arange(8, 9):
        # file = 'people'+str(file)
        file = str(file)
        # file = file+'_23'
        data_item = os.path.join(data_root, file)  # r'D:\FullData\facescape_norm\1'
        print(data_item)
        prt_file = os.path.join(prt_root, file)
        save_data_file = os.path.join(save_data_file_root, file)  # D:\Dataset\Render\1


        if os.path.exists(data_item):  # if this path exits we can render it ;   r'D:\FullData\facescape_norm\1'
            if True:  # if render is exits ; we will pass this process ;
                ...
                for file_name in os.listdir(data_item):  #
                    ...
                    if (houzhui == ".obj"):  # bareteeth.000001.obj
                        ...
                        vertices *= scale
                        '''
                        rndr_noraml.set_mesh(vertices=vertices, faces=faces, norms=normals,
                                             face_normals=faces_normals)
                        ...
                        rndr_noraml.set_norm_mat(scaleMin, vertsMean)
                        ...
                        os.makedirs(os.path.join(save_data_file, file_name, 'normal_jpg'),
                        ...
                        new_path_normal_jpg = os.path.join(save_data_file, file_name,
                                                    'normal_jpg')  # D:\Dataset\Render\people1\bareteeth.00001.obj\normal_jpg

                        for view_id in tqdm(range(0, view_num)):
                            if view_id<25 or view_id>339:

                                R = make_rotate(0, view_id * np.pi / 180, 0)
                                rndr.rot_matrix = R.T

                                rndr_noraml.rot_matrix = R.T
                                cam.sanity_check()
                                rndr.set_camera(cam)
                                rndr.display()
                                ...
                                cv.imwrite(os.path.join(new_path_normal_jpg, '%04d_normal.jpg' % view_id), np.uint8(out_all_f*255))
