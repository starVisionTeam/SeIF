...
from .glm import ortho,perspective


class Camera:
    def __init__(self, width=1600, height=1200,focal=5000, near=0.1, far=40,camera_type='o'):
        # Focal Length
        # equivalent 50mm
        ...
        self.center = np.array([0, 0, 2])#location
    ...
    def get_rotation_matrix(self):
        ...
        self.direction = self.normalize_vector(self.target - self.center)
    ...
    def get_gl_matrix(self):
        ...
        if self.camera_type=='o':
            projective = ortho(-self.width / self.focal_x / 2, self.width / self.focal_x / 2,
                               -self.height / self.focal_y / 2, self.height / self.focal_y / 2,
                               z_near, z_far)
        if self.camera_type=='p':
                projective = perspective(2 * np.arctan(0.5 * self.height / self.focal_y), (self.width * self.focal_y) / (self.height * self.focal_x),
            z_near, z_far)
