...
def testPRT(obj_path, n=40): # 传入的obj_path是C:\code\mutil_process\data\data_obj
    dir_path, obj_fname = os.path.split(obj_path)
    print(dir_path)    # 这个是去除最后一个/后的文件路径  C:\code\mutil_process\data
    print(obj_fname)   # 这个是最后一个文件路径 data_obj

    if dir_path[-1] == '/':
        dir_path = dir_path[:-1]

    # 将prt保存格式为data/data_prt/mouth_008.mat形式
    for file in os.listdir(obj_path):   # 枚举这个目录下的子文件的名字；
        _,houzhui = os.path.splitext(file)  # 获取这个文件后缀的名字，如bareteeth 与obj , 或者 bareteeth 与 mtl
        # print(houzhui)
        if(houzhui==".obj"):  # 这个文件是obj类型的文件，则可以用来计算prt
            file_obj_path = os.path.join(obj_path, file)  # 获取具体的obj文件的总名字；

            name, _ = os.path.splitext(
                file)  # 将mouth_008.obj分割成mouth_008，和 obj，从而方便保存prt数据的目录，保存格式为data/data_prt/mouth_008.mat形式

            PRT, F = computePRT(file_obj_path, n, 2)

            sio.savemat(os.path.join(dir_path, 'data_prt', '%s.mat' % name), {'bounce0': PRT, 'face': F},
                        do_compression=True)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--input', type=str, default=r'C:\code\mutil_process\data\data_obj')
